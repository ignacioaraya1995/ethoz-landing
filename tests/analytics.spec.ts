...
// ─────────────────────────────────────────────────────────────────────────────
// 2. WhatsApp link clicks fire whatsapp_click
// ─────────────────────────────────────────────────────────────────────────────

test.describe('Analytics — WhatsApp trackEvent calls', () => {
	test('WhatsApp link on /contact fires whatsapp_click with source:contact', async ({ page }) => {
		await page.goto('/contact');
		await page.waitForLoadState('networkidle');
		await patchDataLayer(page);

		// Block popup so we stay on the page
		...
		const events = await getCaptured(page, 'whatsapp_click');
		const contactEvents = events.filter((e: any) => e.source === 'contact');
		expect(contactEvents.length).toBeGreaterThanOrEqual(1);
	});

	test('WhatsApp link on /get-started fires whatsapp_click with source:get_started', async ({
		page
	}) => {
		...
		const events = await getCaptured(page, 'whatsapp_click');
		const getStartedEvents = events.filter((e: any) => e.source === 'get_started');
		expect(getStartedEvents.length).toBeGreaterThanOrEqual(1);
	});
});
